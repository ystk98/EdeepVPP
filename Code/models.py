...
class EdeepVPP(nn.Module):
    def __init__(self, vocab_size, emb_dim, out_size=2):
        ...
        self.conv1 = nn.Conv1d(5, 32, kernel_size=7)
        self.conv2 = nn.Conv1d(32, 8, kernel_size=4)
        self.conv3 = nn.Conv1d(8, 8, kernel_size=4)
        ...
        self.fc1 = nn.Linear(in_features=272, out_features=32)
...
class EdeepVPPHybrid(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.lstm(x)
        print(x[0][:, :, -1].shape)
        x = self.fc1(x[0]) # last hidden state, all
        y = self.fc2(x)
...
class ParallelEdeepVPP(nn.Module):
    def __init__(self, vocab_size, emb_dim, out_size=2):
        ...
        self.embed = nn.Embedding(vocab_size, emb_dim)
        self.conv1 = nn.Sequential(
            nn.Conv1d(in_channels=5, out_channels=64, kernel_size=3, padding=2), 
            nn.ReLU(), 
            nn.Conv1d(in_channels=64, out_channels=8, kernel_size=3, padding=2), 
            nn.ReLU(), 
            nn.MaxPool1d(kernel_size=4), 
            nn.Flatten()
        )
        self.conv2 = nn.Sequential(
            nn.Conv1d(in_channels=5, out_channels=64, kernel_size=32, padding=4), 
            nn.ReLU(), 
            nn.MaxPool1d(kernel_size=4), 
            nn.Flatten()
        )
        """
        self.conv3 = nn.Sequential(
            nn.Conv1d(in_channels=5, out_channels=16, kernel_size=64, padding=4), 
            nn.ReLU(), 
        ...
        """
        self.fc = nn.Sequential(
            nn.Linear(in_features=5024, out_features=2048), 
            nn.ReLU(), 
            nn.Dropout(p=0.2), 
